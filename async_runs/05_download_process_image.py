...
async def download_images(urls: list) -> list[Path]:
    ...
    async with httpx.AsyncClient() as client:
        # Use asyncio.TaskGroup to manage concurrent download tasks, we schedule the 
        # download_single_image in event loop to continue running other tasks concurrently.
        # As it is io-bound operation, we can use asyncio.gather to run multiple download tasks concurrently.
...
async def process_images(orig_paths: list[Path]) -> list[Path]:
    ...
    # Use ProcessPoolExecutor to process images in parallel using multiple CPU cores.
    # We execute the process_single_image function in a separate process for each image, 
    # allowing us to take advantage of multiple CPU cores for CPU-bound tasks.
